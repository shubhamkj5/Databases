...
if (target.equals(exportButton)){  		
				...
				cmd="SELECT RBS_ID,count(RBS_ID) as count, Alarm_Description FROM alarms.alarms group by RBS_ID;";
				filename="perRBSexport";
}
else if(target.equals(allAlarmsforRBSButton)) {
	cmd="SELECT * FROM alarms.alarms where RBS_ID="+rBSIDTF.getText();
	filename="IDRBSexport";
}
else {
	cmd="SELECT * FROM alarms.alarms where date>'"+timeTF.getText()+"';";
	filename="dateRBSexport";
}
